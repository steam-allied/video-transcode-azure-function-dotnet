...
namespace converter_service_w_dotnet
{
    public class Mp43gp
    {
        public static readonly string incomingContainerName = "incoming-mp4-n-3gp";
        ...
        public static async Task Run(
          [BlobTrigger("incoming-mp4-n-3gp/{name}", Connection = "AzureWebJobsStorage")] Stream input,
          string name,
          ILogger log)
        {
            ...
            if (fromFormat == ".mp4")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v libx264 -crf 18 -preset medium -c:a libopencore_amrnb -ac 1 -ar 8000 -b:v 1000k -b:a 12.2k -r 15 {outputFile}";
            if (fromFormat == ".3gp")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v libx264 -c:a aac -b:a 128k {outputFile}";
