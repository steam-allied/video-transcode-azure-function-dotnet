...
namespace converter_service_w_dotnet
{
    public class Mp4mkv
    {
        public static readonly string incomingContainerName = "incoming-mp4-n-mkv";
          ...
          ILogger log)
        {
            ...
            if (fromFormat == ".mkv")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v copy -c:a aac -strict experimental {outputFile}";
            process.Start();
            process.WaitForExit();
