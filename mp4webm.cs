...
{
    public class Mp4webm
    {
        public static readonly string incomingContainerName = "incoming-mp4-n-webm";
        ...
        [FunctionName("Mp4webm")]
        public static async Task Run(
          [BlobTrigger("incoming-mp4-n-mkv/{name}", Connection = "AzureWebJobsStorage")] Stream input,
          string name,
          ILogger log)
        {
            ...
            if (fromFormat == ".mp4")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v libvpx -crf 10 -b:v 1M -c:a libvorbis {outputFile}";
            if (fromFormat == ".webm")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v libx264 -c:a aac -strict -2 {outputFile}";
            process.Start();
            process.WaitForExit();
