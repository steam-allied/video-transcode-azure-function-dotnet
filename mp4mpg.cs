...
namespace converter_service_w_dotnet
{
    public class Mp4mpg
    {
        public static readonly string incomingContainerName = "incoming-mp4-n-mpg";
        ...
        public static async Task Run(
          [BlobTrigger("incoming-mp4-n-mpg/{name}", Connection = "AzureWebJobsStorage")] Stream input,
          string name,
          ILogger log)
        {
            ...
            if (fromFormat == ".mp4")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v libx264 -pix_fmt yuv420p -c:a mp2 -b:v 6000k -b:a 192k {outputFile}";
            if (fromFormat == ".mpg")
