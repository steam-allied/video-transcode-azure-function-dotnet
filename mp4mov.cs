...
namespace converter_service_w_dotnet
{
    public class Mp4mov
    {
        public static readonly string incomingContainerName = "incoming-mp4-n-mov";
        ...
        public static async Task Run(
          [BlobTrigger("incoming-mp4-n-mov/{name}", Connection = "AzureWebJobsStorage")] Stream input,
          string name,
          ILogger log)
        {
            ...
            if (fromFormat == ".mp4")
                process.StartInfo.Arguments = $"-i {inputFile} -c:v copy -c:a copy {outputFile}";
            if (fromFormat == ".mov")
