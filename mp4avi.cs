...
namespace converter_service_w_dotnet
{
    public class Mp4avi
    {
        public static readonly string incomingContainerName = "incoming-mp4-n-avi";
        ...
        public static async Task Run(
          [BlobTrigger("incoming-mp4-n-avi/{name}", Connection = "AzureWebJobsStorage")] Stream input,
          string name,
          ILogger log)
        {
            BlobContainerClient containerClient = new BlobContainerClient(connectionString, incomingContainerName);
            ...
            if (fromFormat == ".avi")
                process.StartInfo.Arguments = $"-i {inputFile} -c copy {outputFile}";
